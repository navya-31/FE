...
export const createHotel = async (req,res)=>{
    ...
    try{
        ...
        const updatedOwner = await ownerModel.updateOne({ user_id: o_id }, { $set: { hotel_id: newHotel._id, hotel_name:newHotel.name } });
...
export const updateHotel = async (req,res)=>{
    ...
    const updatedHotel = await hotelModel.findByIdAndUpdate(_id, {...hotel, _id}, {new: true});
...
export const generateBill = async (req, res)=>{
    ...
    try {
        ...
        const billpayers = await tenantModel.updateMany({ hotel_id: id }, 
            { $set: { bill_paid: false } });
        const updatedBillDate = await hotelModel.updateMany({_id: id},
