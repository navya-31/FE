...
export const getMealItemsByHotel = async(req,res) => {
    const {id} = req.params;
    try{     
        const mealItems = await mealItemModel.find({hotel_id: id});
        console.log(mealItems);
        res.status(200).json(mealItems);
    } catch(error){
        res.status(404).json({message: error.message});
...
export const deleteMealItemsByHotelId = async(req,res)=>{
    const id = req.params.id;
    const deletedMealItem = await mealItemModel.deleteMany({hotel_id: id});
